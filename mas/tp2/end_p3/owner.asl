...
+!receive_tasks
   <- .wait(5000);
      !define_reputations; // simplified version
      for ( my_preference(Task, Price) ) {
         ...
         ?best_for_task(Task, Ag);
         .concat("auction_for_", Task, ArtName);
         makeArtifact(ArtName, "tools.AuctionArt", [Task, Price], ArtId);
         .send(Ag, achieve, auction_to_use(Task, ArtName));
      };
      .
...
+!define_reputations
   <-
      for ( all_agents(Ag) ) {
         if (not reputation_of(Ag, _)) {
            Rep = math.floor(math.random(1400));
            +reputation_of(Ag, Rep);
         }
      };
      for ( reputation_of(Ag, Rep) ) {
         .print("Rep of ", Ag, " is ", Rep)
      };
      .
